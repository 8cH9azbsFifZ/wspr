AC_DEFUN([AC_WSPR_SH_DQ], [
  ac_sh_dq="\"`$1 | sed 's/"/\\\\"/g'`\""
])

AC_DEFUN([AC_WSPR_BUILD_INFO], [
# Define build flags and substitute in Makefile.in
# CPPFLAGS
  WSPR_BUILD_CPPFLAGS="-I\$(srcdir) -I\$(srcdir)/include -I\$(srcdir)/irrxml \
-I\$(srcdir)/fileselector"
# CXXFLAGS
  WSPR_BUILD_CXXFLAGS="$PORTAUDIO_CFLAGS $FLTK_CFLAGS $X_CFLAGS $SNDFILE_CFLAGS $SAMPLERATE_CFLAGS \
...
$INTL_CFLAGS $PTW32_CFLAGS $BFD_CFLAGS -pipe -Wall -fexceptions $OPT_CFLAGS $DEBUG_CFLAGS"
  if test "x$target_mingw32" = "xyes"; then
      WSPR_BUILD_CXXFLAGS="-mthreads $WSPR_BUILD_CXXFLAGS"
  fi
# LDFLAGS
  WSPR_BUILD_LDFLAGS="$MAC_UNIVERSAL_LDFLAGS"
  if test "x$target_mingw32" = "xyes"; then
      WSPR_BUILD_LDFLAGS="-mthreads $WSPR_BUILD_LDFLAGS"
  fi
# LDADD
  WSPR_BUILD_LDADD="$PORTAUDIO_LIBS $FLTK_LIBS $X_LIBS $SNDFILE_LIBS $SAMPLERATE_LIBS \
...
# LDADD
  ...
  if test "x$ac_cv_debug" = "xyes"; then
      WSPR_BUILD_CXXFLAGS="$WSPR_BUILD_CXXFLAGS -UNDEBUG"
      WSPR_BUILD_LDFLAGS="$WSPR_BUILD_LDFLAGS $RDYNAMIC"
      FLARQ_BUILD_CXXFLAGS="$FLARQ_BUILD_CXXFLAGS -UNDEBUG"
      FLARQ_BUILD_LDFLAGS="$FLARQ_BUILD_LDFLAGS $RDYNAMIC"
  else
      WSPR_BUILD_CXXFLAGS="$WSPR_BUILD_CXXFLAGS -DNDEBUG"
      FLARQ_BUILD_CXXFLAGS="$FLARQ_BUILD_CXXFLAGS -DNDEBUG"
  fi

  AC_SUBST([WSPR_BUILD_CPPFLAGS])
  AC_SUBST([WSPR_BUILD_CXXFLAGS])
  AC_SUBST([WSPR_BUILD_LDFLAGS])
  AC_SUBST([WSPR_BUILD_LDADD])
...
#define build variables for config.h
  ...
  AC_WSPR_SH_DQ([echo $ac_configure_args])
  AC_DEFINE_UNQUOTED([BUILD_CONFIGURE_ARGS], [$ac_sh_dq], [Configure arguments])

  AC_WSPR_SH_DQ([date])
  AC_DEFINE_UNQUOTED([BUILD_DATE], [$ac_sh_dq], [Build date])

  AC_WSPR_SH_DQ([whoami])
  AC_DEFINE_UNQUOTED([BUILD_USER], [$ac_sh_dq], [Build user])

  AC_WSPR_SH_DQ([hostname])
  AC_DEFINE_UNQUOTED([BUILD_HOST], [$ac_sh_dq], [Build host])

  AC_WSPR_SH_DQ([$CXX -v 2>&1 | tail -1])
  AC_DEFINE_UNQUOTED([BUILD_COMPILER], [$ac_sh_dq], [Compiler])

  AC_WSPR_SH_DQ([echo $WSPR_BUILD_CPPFLAGS $WSPR_BUILD_CXXFLAGS])
  AC_DEFINE_UNQUOTED([WSPR_BUILD_CXXFLAGS], [$ac_sh_dq], [Fldigi compiler flags])
  AC_WSPR_SH_DQ([echo $WSPR_BUILD_LDFLAGS $WSPR_BUILD_LDADD])
  AC_DEFINE_UNQUOTED([WSPR_BUILD_LDFLAGS], [$ac_sh_dq], [Fldigi linker flags])

  AC_WSPR_SH_DQ([echo $FLARQ_BUILD_CPPFLAGS $FLARQ_BUILD_CXXFLAGS])
  AC_DEFINE_UNQUOTED([FLARQ_BUILD_CXXFLAGS], [$ac_sh_dq], [Flarq compiler flags])
  AC_WSPR_SH_DQ([echo $FLARQ_BUILD_LDFLAGS $FLARQ_BUILD_LDADD])
...
AC_DEFUN([AC_WSPR_BUILD_RULES_SILENT], [
...
AC_DEFUN([AC_WSPR_BUILD_RULES],
         [m4_ifdef([AM_SILENT_RULES], [AC_WSPR_BUILD_RULES_SILENT],
                   [AC_SUBST([SILENT_CMDS], [])])])
